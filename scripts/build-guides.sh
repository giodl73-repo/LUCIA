...
[ ! -f "${PROOF}" ] && [ ! -f "${PROOF}.exe" ] && PROOF="${REPO_ROOT}/../../tools-infra/proof/target/debug/proof"
[ ! -f "${PROOF}" ] && [ ! -f "${PROOF}.exe" ] && { echo "proof binary not found. Build the PROOF repo first."; exit 1; }
